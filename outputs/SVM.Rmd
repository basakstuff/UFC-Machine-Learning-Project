...
```
###Drop off columns that are not relevant for the model
```{r}
...
```
###Function to make the Data Set suitable for the model
```{r}
...
svm_classifier <- svm(Winner ~ ., 
                      ...
                      probability=TRUE)
```
###Predictions on unseen testing set
```{r}
...
###Confusion Matrix Visualization (Without Draw Variable)
```{r}
table <- data.frame(confusionMatrix(svmpred, prepared_dataset$test_category)$table)
